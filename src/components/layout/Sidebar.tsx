...
import { Brain, ChartBar, CaretLeft, CaretRight, Database, FileText, Gear, Pulse, Scroll, SealCheck, ShieldCheck, SquaresFour, Target, TrendUp, User, Users } from '@phosphor-icons/react';
...
export const Sidebar: React.FC<SidebarProps> = ({ isCollapsed, onToggleCollapse }) => {
  ...
  const studentNavItems = [
    { title: 'Dashboard', path: '/dashboard', icon: SquaresFour },
    { title: 'Profile', path: '/profile', icon: User },
    { title: 'Resume', path: '/resume', icon: FileText },
    { title: 'Skills Inventory', path: '/skills', icon: SealCheck },
    { title: 'Skill Gap Analysis', path: '/analysis', icon: Brain },
    { title: 'Recommendations', path: '/recommendations', icon: Target },
    { title: 'Market Trends', path: '/market-trends', icon: TrendUp },
    { title: 'Reports & Exports', path: '/reports', icon: ChartBar },
    { title: 'Settings', path: '/settings', icon: Gear }
  ];

  const adminNavItems = [
    { title: 'Admin Overview', path: '/admin', icon: ShieldCheck },
    { title: 'User Management', path: '/admin/users', icon: Users },
    { title: 'Job Ingestion Sources', path: '/admin/job-sources', icon: Database },
    { title: 'Platform Analytics', path: '/admin/analytics', icon: ChartBar },
    { title: 'System Health', path: '/admin/system', icon: Pulse },
    { title: 'Audit Logs', path: '/admin/logs', icon: Scroll },
    { title: 'Student Portal View', path: '/dashboard', icon: SquaresFour }
  ];
  ...
  return (
    ...
    >
      ...
      >
        {isCollapsed ? <CaretRight className="w-3.5 h-3.5" /> : <CaretLeft className="w-3.5 h-3.5" />}
      </button>

      {/* Brand Header */}
      <div
        className={cn(
          'h-16 flex items-center border-b border-slate-800/40 shrink-0 px-4',
        ...
        >
          ...
          {!isCollapsed && (
            <div className="flex flex-col truncate">
              ...
              <span className="text-[10px] font-semibold text-accent-500 uppercase tracking-wider">
                Enterprise AI v1.0
              </span>
            </div>
          )}
        ...
        {navItems.map(item => {
          ...
          return (
            <NavLink
              ...
              className={({ isActive }) =>
                cn(
                  'flex items-center py-2.5 rounded-none text-xs font-medium transition-colors group relative border-l-[3px]',
                  isCollapsed ? 'justify-center px-0' : 'space-x-3 px-4',
                  isActive
                    ? 'bg-[#1C2541] text-white border-[#10B981] font-semibold'
                    : 'text-slate-400 border-transparent hover:text-white hover:bg-[#1C2541]/50'
                )
              }
            >
              {({ isActive }) => (
      ...
      <div className={cn('p-3 border-t border-slate-800/40 bg-[#070D1E] shrink-0', isCollapsed && 'flex justify-center')}>
